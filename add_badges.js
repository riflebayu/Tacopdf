...
// I will insert these keys under 'footer.copyright' in every language block
const insertions = {
    'en': {
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('"footer.copyright"')) {
        // find which lang we are in by scanning backwards
        let currentLang = 'en';
        for (let j = i; j >= 0; j--) {
            const match = lines[j].match(/^\s*"([a-z]{2})":\s*\{/);
            if (match) {
                currentLang = match[1];
                break;
            }
        ...
        if (ins) {
            ...
            newLines.push(`    "header.badge.fastSecure": "${ins['header.badge.fastSecure']}",`);
        }
    }
}
