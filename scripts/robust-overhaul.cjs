...
fs.writeFileSync('scripts/temp-trans.cjs', `module.exports = ${objStr}`);

const translations = require('./temp-trans.cjs');
...
for (const lang in translations) {
  ...
  // Add new keys if we have them for this lang, otherwise fallback to EN
  const newTrans = watermarkTranslations[lang] || watermarkTranslations['en'];
...
console.log('Restored pretty translations.ts with perfect watermark overhaul!');
