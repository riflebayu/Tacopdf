...
for (const lang of Object.keys(replacements)) {
    ...
    const startIdx = content.indexOf('"privacy.doc_title":', langStart);
    if (startIdx === -1) {
        console.log("privacy.doc_title not found for " + lang);
        continue;
    }
    
    // Instead of full regex match, let's find the exact string bounds safely.
    // The last key is either privacy.p7.text or privacy.p6.text.
    // Let's find privacy.p6.text and privacy.p7.text. We pick the one that comes later.
    let endKeyIdx = content.indexOf('"privacy.p7.text":', startIdx);
    if (endKeyIdx === -1 || endKeyIdx > startIdx + 10000) {
        endKeyIdx = content.indexOf('"privacy.p6.text":', startIdx);
    }
    
    if (endKeyIdx !== -1 && endKeyIdx < startIdx + 10000) {
        // Find the end of this line (e.g., after the closing quote and possible comma)
        let lineEndIdx = content.indexOf('\\n', endKeyIdx);
        if (lineEndIdx !== -1) {
            // Also include \\r if it exists
            if (content[lineEndIdx - 1] === '\\r') {
               // But we will just substring up to lineEndIdx
            }
            const block = content.substring(startIdx, lineEndIdx);
            
            // Reconstruct block exactly
            content = content.substring(0, startIdx) + replacements[lang] + (content[lineEndIdx - 1] === ',' ? ',' : '') + content.substring(lineEndIdx);
            console.log("Replaced successfully for " + lang);
        }
    } else {
        console.log("Could not find end key for " + lang);
    }
}
