...
const translations = {
  en: `    "tool_name.compress": "Compress PDF",
  ...
  id: `    "tool_name.compress": "Kompres PDF",
  ...
  pt: `    "tool_name.compress": "Comprimir PDF",
  ...
  fr: `    "tool_name.compress": "Compresser PDF",
  ...
  de: `    "tool_name.compress": "PDF Komprimieren",
  ...
  es: `    "tool_name.compress": "Comprimir PDF",
...
// We will split by lines and insert right after the match of 'workspace.merge.upload_more'
for (const [lang, additions] of Object.entries(translations)) {
  const lines = content.split('\\n');
  let langStart = -1;
  // find language block start
  ...
  if (langStart !== -1) {
    // find upload_more inside this block
    for (let j = langStart; j < lines.length; j++) {
      if (lines[j].includes('workspace.merge.upload_more')) {
        // found it, let's insert after this line
        lines.splice(j + 1, 0, additions);
        content = lines.join('\\n');
        break;
      }
    }
